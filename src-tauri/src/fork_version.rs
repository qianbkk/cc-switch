...
//!
//! - **发布构建**（编译时注入了 `CC_SWITCH_FORK_RELEASE_TAG`，形如 `m3.19.1-3`）：
...
mod tests {
    ...
    fn release_build_uses_embedded_tag() {
        ...
        let info = fork_version_info("3.19.1");
        // 无论编译环境如何，字段结构完整、base_version 正确
        assert_eq!(info.base_version, "3.19.1");
